...
function useRoomSession() {
  ...
  // react-doctor-disable-next-line query-mutation-missing-invalidation -- created room is consumed directly (payload.roomCode -> room websocket); no cached query stores it
  ...
  // react-doctor-disable-next-line query-mutation-missing-invalidation -- invite is delivered to the invitee; badge/notifications own the sync for recipients, sender has no cached invite query
  ...
  const switcherTurn = isSwitcherCoopActive ? (isMyTurnInSwitcher ? "me" : "teammate") : null;
  ...
  const rivalProgress = useMemo(() => {
    ...
    return candidateProgress.reduce((best, candidate) => {
      ...
      return candidate.currentWordIndex > best.currentWordIndex ? candidate : best;
    });
  ...
  const invitingFriendIdSet = useMemo(() => new Set(invitingFriendIds), [invitingFriendIds]);
  const pendingInviteFriendIdSet = useMemo(
    () => new Set(pendingInviteFriendIds),
...
function ModeSelectCard({ icon, label, title, selected, onSelect }) {
  return (
    <m.button
      type="button"
      onClick={onSelect}
      ...
      >
        <CardHeader className="gap-2 pb-3">
          <div className="flex items-center gap-2">
            <div className={`rounded-md p-2 ${selected ? "" : "bg-muted"}`}>
              {icon}
            </div>
            <p className={`font-mono text-xs uppercase tracking-[0.15em] ${selected ? "text-primary-foreground/80" : "text-muted-foreground"}`}>
              {label}
            </p>
          </div>
          <CardTitle className={`font-sans text-lg ${selected ? "text-primary-foreground" : ""}`}>{title}</CardTitle>
        </CardHeader>
      </Card>
    </m.button>
  );
...
function JoinRoomForm({ joinCode, setJoinCode, joinRoom, wsStatus }) {
  return (
    <m.div
      key="join"
      initial={{ opacity: 0, y: 10 }}
    ...
    >
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <ModeSelectCard
          ...
          onSelect={() => setEntryMode("join")}
        />
      </div>

      {/* Create/Join Forms */}
      {/* react-doctor-disable-next-line motion-animate-presence-must-outlive-child -- AP stays mounted while children swap between create/join (corrected form); only the whole forms section unmounts on joining a room, a screen-level transition where no exit anim is expected */}
            ...
            <div className="flex items-center gap-3">
              ...
              {isLobbyState && (
                <>
                  ...
                  >
{/* react-doctor-disable-next-line motion-animate-presence-must-outlive-child -- AP stays mounted while the icon swaps between check/copy (corrected form); it only unmounts when leaving the lobby, a screen-level transition */}
                    <AnimatePresence mode="wait">
                      {copied ? (
                        <m.div
                          key="check"
                  ...
                  {/* react-doctor-disable-next-line motion-animate-presence-must-outlive-child -- AP stays mounted while the 'Copied!' hint toggles on/off (corrected form); only unmounts when leaving the lobby */}
                  <AnimatePresence>
                    {copied && (
                      <m.span
        ...
        {friendsForInvite.length === 0 ? (
          ...
          <div className="grid gap-2 md:grid-cols-2">
            {friendsForInvite.map((friend) => {
              ...
              const isInviting = invitingFriendIdSet.has(friend.id);
              const alreadyInvited = pendingInviteFriendIdSet.has(friend.id);
...
function RoomCountdownCard({ countdown }) {
  return (
    <m.div
      ...
      <div className="grid grid-cols-2 gap-4">
        <Card className="border-primary/30 bg-primary/5">
          ...
          <CardContent>
            ...
            <div className="font-mono text-xs text-muted-foreground">
              Word {(switcherTurn ? myTeamTurnState?.currentWordIndex : myProgress.currentWordIndex) + 1} / {wordCount}
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <div className="font-mono text-xs text-muted-foreground">
              Word {rivalProgress.currentWordIndex + 1} / {wordCount}
            </div>
          </CardContent>
        </Card>
      </div>

      <Card className={switcherTurn === "me" ? "border-primary/60 ring-1 ring-primary/20" : ""}>
        <CardHeader className="pb-4">
          ...
          {switcherTurn ? (
            <div className="mt-2 flex items-center justify-between">
              <span className={`inline-flex items-center rounded-full px-2 py-1 text-[11px] font-mono uppercase tracking-[0.08em] ${
                ...
                {switcherTurn === "me" ? "Your Turn" : "Teammate Turn"}
              </span>
              {switcherTurn === "teammate" && activeTeammate ? (
        ...
        <CardContent className="space-y-6">
          ...
          <input
            ...
            disabled={switcherTurn === "teammate"}
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            ...
            <span className="font-mono text-xs text-primary">
              {Math.round((myProgressValue / (wordCount || 1)) * 100)}%
            </span>
          </div>
          <div className="h-2 w-full overflow-hidden rounded-full bg-secondary">
            <m.div
              ...
              initial={{ scaleX: 0 }}
              animate={{ scaleX: myProgressValue / (wordCount || 1) }}
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            ...
            <span className="font-mono text-xs text-destructive">
              {Math.round((rivalProgressValue / (wordCount || 1)) * 100)}%
            </span>
          </div>
          <div className="h-2 w-full overflow-hidden rounded-full bg-secondary">
            <m.div
              ...
              initial={{ scaleX: 0 }}
              animate={{ scaleX: rivalProgressValue / (wordCount || 1) }}
              transition={{ duration: 0.3 }}
            />
          </div>
        </div>
          ...
          <CardContent className="space-y-3">
            <div className="grid gap-2 rounded-md border border-border/60 bg-card/50 px-3 py-2 text-xs sm:grid-cols-3">
              <div className="flex items-center justify-between sm:block">
                ...
                <span className="font-mono font-semibold">{myProgressScore} pts</span>
              </div>
              <div className="flex items-center justify-between sm:block">
                    ...
                    <div className="space-y-1 p-2">
                      {(team.members || []).map((member) => (
                        ...
                        >
                          ...
                          <span className="text-right font-mono text-xs text-muted-foreground">
                            {Math.min(Number(member.progress || 0), wordCount)} w
                          </span>
                        </div>
                      ))}
                    </div>
    ...
    <div className="flex h-full flex-col gap-6">
      ...
      {isPlaying ? (
        <RoomPlayingScreen
          ...
          switcherTurn={switcherTurn}
          myTeamTurnState={myTeamTurnState}
          activeTeammate={activeTeammate}
          nextTeammateLabel={nextTeammateLabel}
          currentWord={currentWord}
          liveSwitcherInput={liveSwitcherInput}
          gameInput={gameInput}
          ...
          timeLeft={timeLeft}
          wordCount={game?.words?.length || 0}
          myTeamName={myTeamName}
          rivalTeamName={rivalTeamName}
          myProgressValue={myProgressValue}
          rivalProgressValue={rivalProgressValue}
        />
      ) : null}
      ...
      {gameResult ? (
        <RoomResultsScreen
          ...
          isCoopResult={isCoopResult}
          winningTeam={winningTeam}
          isMeWinner={gameResult.winnerId === currentUser?.uid}
          myProgressScore={myProgress.score}
          myWordsCompleted={myWordsCompleted}
      ...
      {!isInRoom ? (
        ...
        >
          ...
          {isLeader && isLobbyState ? (
            <InviteFriendsCard
              friendsForInvite={friendsForInvite}
              invitingFriendIdSet={invitingFriendIdSet}
              pendingInviteFriendIdSet={pendingInviteFriendIdSet}
            ...
            <div className="grid gap-6 lg:grid-cols-2">
              {settingsForm.gameMode === "ffa" ? (
                ...
                <CoopTeamsCard
                  ...
                  teamNameLocks={teamNameLocks}
                  updateTeamName={updateTeamName}
                  commitTeamName={commitTeamName}
                  assignMeToTeam={assignMeToTeam}
              ...
              <RoomSettingsCard
                ...
                setSettingsForm={setSettingsForm}
                updateSettingsField={updateSettingsField}
                updateLeaderSettings={updateLeaderSettings}
              />
            </div>
          )}

          {roomState?.state === "countdown" ? (
            <RoomCountdownCard countdown={countdown} />
          ) : null}
        </m.div>
      )}
      </>
